...
class Library {
    ...
    func deleteBook(id: String) -> Bool {
        if ids.contains(id) {
            ...
            for (i, id_in_list) in ids.enumerated() {
                if id_in_list == id {
                    ids.remove(at: i)
                }
            }
    ...
    func listBooksByName(searchString: String) -> [String] {
        var list_of_books: [String] = []
        var books_numbers: [Int] = []
        
        for (number, book) in books.enumerated() {
            if book.name.contains(searchString) {
                list_of_books.append(book.name)
                books_numbers.append(number)
            }
        }
        
        var i = 0
        while i < list_of_books.count && i < 11 {
            if list_of_books[i] == list_of_books[i + 1] {
                list_of_books[i] = books[books_numbers[i]].author + list_of_books[i]
                list_of_books[i + 1] = "\(books[books_numbers[i + 1]].author) - \(list_of_books[i + 1])"
                i += 1
            }
        }
    ...
    func listBooksByAuthor(searchString: String) -> [String] {
        ...
        authors = authors.sorted(by: <)
        var i = 0
        while i < authors.count && i < 11 {
            for author_name in authors {
                for book in books {
                    if book.author == author_name {
                        books_in_order.append(book.name)
                        i += 1
                    }
                }
            }
